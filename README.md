...
- Streaming chat requests use `text/event-stream`. Non-stream chat requests preserve OpenAI-compatible `application/json`, even when `heartbeat.mode = "sse"` is configured. Correctness-first shielding holds either response before acceptance: no heartbeat or other downstream bytes are emitted while an upstream attempt, retry, or local recovery can still change the result.
- If the same normalized input fingerprint repeats within the loop-guard window, the accepted downstream response switches to buffered `application/json`. It remains byte-free before acceptance.
...
- Before a buffered shielded completion is accepted, conservative deterministic checks recognize explicit mechanical Prose constraints such as acrostics, telestichs, lipograms, exact line/word counts, fixed refrains, and snowball word lengths. A detected violation consumes the normal retry budget and injects a targeted repair hint; it deliberately restarts at the first, highest-quality retry-ladder rung rather than trading thinking quality for latency. Unrecognized prompts, explicitly detected self-contradictory constraints, exhausted retry budgets, and direct-relay streaming responses are left unchanged.
- `heartbeat.mode` is recorded as the raw configured preference. Actual framing is derived from the request contract: streaming requests use SSE, while non-stream requests use buffered JSON (`json-whitespace` is retained as a framing label but emits no pre-commit whitespace). The effective pre-commit liveness mode is always `held`; observability records configured, framing, and effective modes plus the actual emitted heartbeat count (`0`).
...
- `heartbeat.mode = "sse"` is the default configured preference, not the accepted non-stream framing. Shielded non-stream chat still returns buffered OpenAI-compatible `application/json`; streaming requests use SSE. Neither path emits pre-commit heartbeats while recovery or replay remains possible, so every downstream operator idle timeout must exceed the bounded attempt, recovery, and replay interval.

...
- `heartbeat.mode = "sse"` is the default accepted-response framing for shielded non-stream chat. It does not protect the pre-commit hold from intermediary idle timeouts: every downstream operator timeout must exceed the bounded attempt, recovery, and replay interval. Repeated normalized inputs use buffered JSON framing.

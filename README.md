...
- The default accepted downstream response is `text/event-stream` with one `event: final` containing the OpenAI-compatible chat completion JSON. Correctness-first shielding holds the response before acceptance: no heartbeat or other downstream bytes are emitted while an upstream attempt, retry, or local recovery can still change the result.
...
- `heartbeat.mode` selects accepted-response framing, but the current effective pre-commit liveness mode is always `held`; observability records both configured and effective mode plus the actual emitted heartbeat count (`0`). `heartbeat.mode = "disabled"` keeps the legacy buffered JSON response.

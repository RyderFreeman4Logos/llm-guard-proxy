...
use super::{
    ...
    LocalRecoveryCause, LocalRecoveryGate, LocalRecoveryPolicy, Ordering,
    UpstreamStallRecoveryCoordinator, applied_local_recovery_gate, local_recovery_completed_ready,
    local_recovery_downstream_commit_observed, local_recovery_metadata,
    local_recovery_permits_retry, run_local_recovery_for_profile_with_downstream_commit,
...
pub(super) async fn gate(
    ...
    let recovery_metadata = run_local_recovery_for_profile_with_downstream_commit(
        context.policy,
        context.coordinator,
        context.client,
        context.base_url.to_owned(),
        cause,
        context.episode_timeout,
        context.downstream_commit_signal.cloned(),
    )
    .await;

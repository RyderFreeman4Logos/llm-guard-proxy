...
async fn guard_listener_rejects_reserved_forced_alias_upstream_model_before_forwarding() {
    ...
    let ordinary_alias = "unconfigured-public-alias";
    let response = proxy_handler(
        State(guard_state),
        Request::builder()
            .method(Method::POST)
            ...
            .body(Body::from(format!(
                r#"{{"model":"{ordinary_alias}","messages":[]}}"#
            )))
            .expect("ordinary alias request should build"),
    )
    .await;
    assert_eq!(response.status(), StatusCode::OK);
    let observed = fake.recv_next().await;
    let observed: serde_json::Value =
        serde_json::from_slice(&observed.body).expect("ordinary alias request should be JSON");
    assert_eq!(
        observed["model"], ordinary_alias,
        "ordinary aliases must not activate the forced canonical rewrite"
    );
}
...
async fn guard_listener_hot_reload_updates_reserved_identity_with_alias_generation() {
    ...
    let changed = std::fs::read_to_string(proxy.root.join("config.toml"))
        ...
        .replace(
            "reserved_ingress_model_ids = [\"abliterated-qwen-latest-27b-nvfp4\", \"aeon\", \"aeon-ultimate\"]",
            "reserved_ingress_model_ids = [\"test-reloaded-canonical\", \"aeon\", \"aeon-ultimate\"]",
        )
        .replace(
            "upstream_model = \"abliterated-qwen-latest-27b-nvfp4\"",
            "upstream_model = \"test-reloaded-canonical\"",
        );
...
#[test]
fn forced_alias_rename_with_server_port_change_retains_one_generation() {
    const OLD_ALIAS: &str = "old-public-alias";
    const NEW_ALIAS: &str = "new-public-alias";

    let root = unique_test_dir("forced-alias-mixed-reload-270");
    fs::create_dir_all(&root).expect("test root should be created");
    let path = root.join("config.toml");
    let current = format!(
        r#"
...
"#
    ...
    let replacement = path.with_extension("next");
    fs::write(&replacement, requested).expect("requested generation should be written");
    fs::rename(&replacement, &path).expect("requested generation should be published");
    let outcome = manager.reload().expect("valid reload should be accepted");
    assert!(!outcome.applied);
    ...
    let live = manager.handle().snapshot().expect("live snapshot");
    assert_eq!(live.server.port, 18009);
    let mut routed_profile = assert_forced_alias_generation(&live, OLD_ALIAS, NEW_ALIAS, "low");
    assert_eq!(routed_profile.name, "forced-alias-route");
    let request = Bytes::from(format!(r#"{{"model":"{OLD_ALIAS}","messages":[]}}"#));
    let (policy, body) = forced_model_alias_policy_body(
        &live,
        Some(OLD_ALIAS),
        &Method::POST,
    ...
    assert_eq!(
        policy
            .expect("retained forced alias policy")
            .reasoning_effort
            .as_deref(),
        Some("low")
    );
...
{FORCED_MODEL_ALIAS_PROFILES_CONFIG}"#
    ...
    for (alias, thinking, budget, reasoning_effort, temperature, top_p, presence_penalty) in [
        (
            "abliterated-qwen-latest-27b-none",
            false,
            None,
        ...
        for stream in [false, true] {
            let response = proxy
                ...
                .header(CONTENT_TYPE, "application/json")
                .body(format!(
                    r#"{{"model":"{alias}","messages":[],"stream":{stream},"reasoning_effort":"caller","chat_template_kwargs":{{"reasoning_effort":"caller"}},"temperature":9,"top_p":9,"top_k":9,"min_p":9,"presence_penalty":9,"frequency_penalty":9,"repetition_penalty":9}}"#
                ))
                .send()
                .await

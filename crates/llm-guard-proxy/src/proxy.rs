...
fn client_response_model_alias<'profile>(
    profile: &'profile UpstreamProfileConfig,
    client_model: Option<&'profile str>,
) -> Option<&'profile str> {
    profile
        .upstream_model
        .as_deref()
        .zip(client_model)
        .map(|(_upstream_model, client_model)| client_model)
}

fn response_model_rewrite_mode(headers: &HeaderMap) -> Option<ResponseModelRewriteMode> {
    let content_type = headers.get(CONTENT_TYPE)?.to_str().ok()?;
    let media_type = content_type.split(';').next()?.trim();
    if media_type.eq_ignore_ascii_case("application/json") {
        return Some(ResponseModelRewriteMode::Json);
    }
    if media_type.eq_ignore_ascii_case("text/event-stream") {
        return Some(ResponseModelRewriteMode::OpenAiSse);
    }
    None
}
...
impl<S> ResponseModelRewriteBody<S> {
    fn new(stream: S, mode: ResponseModelRewriteMode, client_model: String) -> Self {
        Self {
...
impl<S, E> Stream for ResponseModelRewriteBody<S>
where
...
struct GenericForwardContext<'request> {
    ...
    local_recovery_attempts: AtomicU64,
}
...
async fn forward_generic_openai_request(
    ...
    let sent_upstream_response = send_generic_upstream_attempt(&context, 1).await;
    let mut sent_upstream_response =
        generic_recovery::complete(&context, sent_upstream_response).await?;
    if sent_upstream_response.response.status() == StatusCode::TOO_MANY_REQUESTS {
        retry_after::sanitize(
            sent_upstream_response.response.headers_mut(),
...
async fn send_generic_upstream_attempt(
    context: &GenericForwardContext<'_>,
    attempt_number: u32,
    ...
    let (transformed_request_headers, mut attempt_request_metadata) =
        prepare_generic_attempt_request(context, attempt_number);
...
/// Associates a watchdog wrapper task with the exact recovery episode its
/// inner local-recovery future started or joined. `0` means the wrapper failed
/// before it observed an episode, so collector cleanup must be a no-op.
#[derive(Debug)]
struct WatchdogRecoveryTask {
    profile: String,
    recovery_episode_id: Arc<AtomicU64>,
}
...
impl WatchdogRecoveryTask {
    fn observed_recovery_episode_id(&self) -> Option<u64> {
        Self::observed_recovery_episode_id_from(&self.recovery_episode_id)
    }

    fn observed_recovery_episode_id_from(recovery_episode_id: &AtomicU64) -> Option<u64> {
        match recovery_episode_id.load(Ordering::Acquire) {
            0 => None,
            recovery_episode_id => Some(recovery_episode_id),
        }
    }
}
...
async fn run_stuck_engine_watchdog(
    ...
    let mut recovery_task_profiles: HashMap<tokio::task::Id, WatchdogRecoveryTask> = HashMap::new();
    while !shutdown.is_shutting_down() {
        ...
        let Ok(snapshot) = config.snapshot() else {
            if wait_for_watchdog_interval(&shutdown, Duration::from_secs(60)).await {
                break;
            }
            continue;
        };
        ...
        if wait_for_watchdog_interval(&shutdown, Duration::from_secs(1)).await {
            break;
        }
    }
...
async fn drain_watchdog_recoveries(
    ...
    recovery_task_profiles: &mut HashMap<tokio::task::Id, WatchdogRecoveryTask>,
    ...
    while let Some(result) = recovery_tasks.join_next_with_id().await {
        match result {
            ...
            Err(error) => {
                if let Some(task) = recovery_task_profiles.remove(&error.id()) {
                    recovering_profiles.remove(&task.profile);
                    record_watchdog_recovery_task_failure(local_recovery, &task).await;
                }
            }
        }
    }
...
async fn record_watchdog_recovery_task_failure(
    local_recovery: &LocalRecoveryCoordinatorSet,
    task: &WatchdogRecoveryTask,
) {
    let coordinator = local_recovery.coordinator_for(&task.profile);
    let metadata = BTreeMap::from([(
        String::from("local_recovery_status"),
        String::from("task_failed"),
    )]);
    if let Some(recovery_episode_id) = task.observed_recovery_episode_id() {
        let _aborted =
            abort_local_recovery_episode(&coordinator, recovery_episode_id, metadata).await;
    }
    coordinator
        .watchdog_recovery_task_failures
        .fetch_add(1, Ordering::Relaxed);
    eprintln!(
        "llm_guard_proxy_stuck_watchdog profile={} event=recovery_task_failed",
        task.profile
    );
}

/// Contains the child represented by an inner watchdog `JoinError`. The wrapper
/// may observe the error after a later recovery has started, so terminalization
/// is compare-and-act on the child episode rather than the coordinator's
/// currently active handle.
async fn watchdog_recovery_join_error_metadata(
    ...
    if let Some(recovery_episode_id) = recovery_episode_id {
        let _aborted =
            abort_local_recovery_episode(coordinator, recovery_episode_id, recovery.clone()).await;
    }
    recovery
}
...
async fn run_watchdog_recovery(
    ...
    coordinator: Arc<UpstreamStallRecoveryCoordinator>,
    endpoint: LocalRecoveryEndpoint,
    shutdown: Arc<ShutdownGate>,
    recovery_episode_id: Arc<AtomicU64>,
    ...
    let recovery_episode_observer = Arc::clone(&recovery_episode_id);
    ...
    let mut recovery = tokio::select! {
        result = &mut local_recovery => match result {
            ...
            Err(error) => watchdog_recovery_join_error_metadata(
                &coordinator,
                WatchdogRecoveryTask::observed_recovery_episode_id_from(&recovery_episode_id),
                error,
            ).await,
        },
        () = shutdown_gate.cancelled() => {
            ...
            if let Some(recovery_episode_id) =
                WatchdogRecoveryTask::observed_recovery_episode_id_from(&recovery_episode_id)
            {
                let _aborted = abort_local_recovery_episode(
                    &coordinator,
                    recovery_episode_id,
                    recovery.clone(),
                )
    ...
    if recovery
        ...
        && let Some(recovery_episode_id) =
            WatchdogRecoveryTask::observed_recovery_episode_id_from(&recovery_episode_id)
        && let Some(completed) =
            wait_for_started_local_recovery_result(&coordinator, recovery_episode_id).await
    {
        recovery = completed;
    }
...
async fn wait_for_started_local_recovery_result(
    ...
    wait_for_started_local_recovery_result_after_state_check(
        coordinator,
        recovery_episode_id,
        || {},
    )
...
async fn wait_for_started_local_recovery_result_after_state_check<F>(
    coordinator: &UpstreamStallRecoveryCoordinator,
    recovery_episode_id: u64,
    after_state_check: F,
) -> Option<BTreeMap<String, String>>
    ...
    F: FnOnce(),
{
...

    {
        ...
        if let Some(result) = state.completed_recovery_result(recovery_episode_id) {
    ...
    if timeout(Duration::from_millis(250), notified).await.is_err() {
        let state = coordinator.state.lock().await;
        return state
            .completed_recovery_result(recovery_episode_id)
            .cloned();
    }
    let state = coordinator.state.lock().await;
    state
...
async fn forward_upstream_response(
    ...
    let mut upstream_headers = response_parts.upstream_headers.clone();
    ...
    let response_model_rewrite = client_response_model_alias(
        dispatch.upstream_profile,
        response_parts.model_id.as_deref(),
    )
...
impl RequestDeadline {
    ...
    fn instant(self) -> Instant {
        checked_instant_add(self.started_at, self.max_duration)
    }
...
fn shielded_liveness_stream_response(
    ...
    let upstream_content_type = started
        ...
        .get(CONTENT_TYPE)
        .map(header_value);
    let mut response_headers = shielded_chat_stream_response_headers(
        &started.info.upstream_headers,
        runtime.liveness.mode,
    );
    let response_model_alias =
        client_response_model_alias(&runtime.upstream_profile, runtime.model_id.as_deref())
            .map(str::to_owned);
    if response_model_alias.is_some() {
        response_headers.remove(CONTENT_LENGTH);
    }
    let extra_metadata =
        effective_liveness::response_metadata(&runtime.liveness, upstream_content_type);
        ...
        if started.info.upstream_status == StatusCode::TOO_MANY_REQUESTS {
                ...
                && wait_for_rate_limit_retry(runtime, delay).await
            {
                let failure = status_failure(
                    &started.info,
                    ShieldedRetryCause::TransientUpstreamStatus,
                    "bounded upstream rate limit before shielded stream",
                );
                attempt_records.push(attempt_failure_record(
                    &failure,
                ...
                return ShieldedAttemptStep::Retry {
                    ...
                    retry_cause: Some(ShieldedRetryCause::TransientUpstreamStatus),
                };
            }
            retry_after::sanitize(
                &mut started.info.upstream_headers,
...
async fn wait_for_rate_limit_retry(runtime: &ShieldedRetryRuntime, delay: Duration) -> bool {
    let Some(remaining) = runtime.request_deadline.remaining() else {
        return false;
    };
    if delay >= remaining {
        return false;
    ...
    tokio::select! {
        ...
        () = tokio::time::sleep(delay) => !runtime.downstream_drop_signal.is_dropped(),
    }
}
...
async fn aggregate_native_json_shielded_attempt(
    ...
    let ShieldedStartedAttempt { info, response, .. } = started;
...
async fn local_recovery_gate(
    ...
    precommit_recovery::gate(
        precommit_recovery::Context {
            ...
            base_url: runtime.upstream_profile.primary_base_url(),
            profile_name: &runtime.upstream_profile.name,
            attempts: &runtime.local_recovery_attempts,
...
fn local_recovery_metadata(
    ...
    BTreeMap::from([
        (
            ...
            policy.is_configured().to_string(),
        ),
        (
...
async fn run_local_recovery_for_profile_observing(
    ...
    if state.running {
        ...
        drop(state);
        return wait_for_local_recovery_result_with_options(
            policy,
            coordinator,
            true,
            recovery_episode_id,
    ...
    let now = Instant::now();
    if let Some(metadata) = local_recovery_admission_failure(policy, &mut state, now) {
        return metadata;
    }
...
fn local_recovery_admission_failure(
    policy: &LocalRecoveryPolicy,
    state: &mut UpstreamStallRecoveryState,
    now: Instant,
) -> Option<BTreeMap<String, String>> {
...
async fn wait_for_local_recovery_result_with_options(
    policy: &LocalRecoveryPolicy,
    coordinator: &Arc<UpstreamStallRecoveryCoordinator>,
    ...
    wait_for_local_recovery_result(
        policy,
        coordinator,
        joined_inflight,
        recovery_episode_id,
...
const LOCAL_RECOVERY_COMPLETION_GUARD: Duration = Duration::from_secs(1);

fn local_recovery_completion_timeout(policy: &LocalRecoveryPolicy) -> Duration {
    policy
        .restart_timeout
        .saturating_add(policy.readiness_deadline)
        .saturating_add(LOCAL_RECOVERY_COMPLETION_GUARD)
}

fn minimum_optional_timeout(first: Option<Duration>, second: Option<Duration>) -> Option<Duration> {
    match (first, second) {
...
fn spawn_local_recovery_terminalizer(
    coordinator: &Arc<UpstreamStallRecoveryCoordinator>,
    recovery_episode_id: u64,
...
async fn abort_local_recovery_episode(
    coordinator: &UpstreamStallRecoveryCoordinator,
    recovery_episode_id: u64,
    metadata: BTreeMap<String, String>,
) -> bool {
...
async fn wait_for_local_recovery_result(
    ...
    let recovery_deadline = *state.recovery_deadline.get_or_insert_with(|| {
        checked_instant_add(recovery_started, local_recovery_completion_timeout(policy))
    });
    let deadline = caller_timeout.map_or(recovery_deadline, |timeout| {
        recovery_deadline.min(checked_instant_add(Instant::now(), timeout))
    });
    drop(state);
    loop {
        let notified = coordinator.notify.notified();
        ...
        let state = coordinator.state.lock().await;
        if let Some(result) = state.completed_recovery_result(recovery_episode_id) {
            return completed_local_recovery_metadata(Some(result), joined_inflight);
        }
        if state.active_recovery_episode_id != Some(recovery_episode_id) {
            return completed_local_recovery_metadata(None, joined_inflight);
        }
        drop(state);
...
#[cfg(test)]
async fn finish_active_upstream_stall_recovery_for_test(
    coordinator: &UpstreamStallRecoveryCoordinator,
    metadata: BTreeMap<String, String>,
) {
    let recovery_episode_id = coordinator
        .state
        .lock()
        .await
        .active_recovery_episode_id
        .expect("test recovery must own an active episode");
    let _published =
        finish_upstream_stall_recovery(coordinator, recovery_episode_id, metadata).await;
}
...
fn shielded_attempt_upstream_timeout(runtime: &ShieldedRetryRuntime) -> Duration {
    runtime
        .request_deadline
        .remaining()
...
fn add_retry_request_metadata(
    ...
    metadata.insert(
        String::from("retry_max_retry_after_secs"),
        policy.max_retry_after.as_secs().to_string(),
    );
    metadata.insert(
        String::from("retry_anti_loop_hint_enabled"),
        policy.anti_loop_hint_enabled.to_string(),
...
fn retry_cause_for_upstream_status(status: reqwest::StatusCode) -> Option<ShieldedRetryCause> {
    if matches!(status.as_u16(), 408 | 500 | 502 | 503 | 504) {
        Some(ShieldedRetryCause::TransientUpstreamStatus)
    } else {
        None
    }
...
fn terminal_forward_failure(
    ...
    let disabled_policy = ShieldedRetryPolicy {
        ...
        max_retry_after: Duration::from_secs(RetryConfig::default().max_retry_after_secs),
...
fn shielded_retry_error_response(
    ...
    failure
        .response_metadata
        .extend(effective_liveness::response_metadata(
            &runtime.liveness,
            None,
        ));
    let observer = shielded_retry_observer(
...
async fn shielded_retry_terminal_forward_response(
    ...
    let observer = shielded_retry_observer(
        ...
        ShieldedRetryObserverInput {
            ...
            extra_response_metadata: liveness_metadata,
            raw_payloads: RawPayloads::default(),
...
async fn shielded_retry_direct_relay_response(
    ...
    let mut upstream_headers = outcome.started.info.upstream_headers.clone();
    let request_path = runtime.downstream_uri.path().to_owned();
    let request_id = runtime.request_id.clone();
    let malformed_counter = runtime.malformed_response_counter.clone();
...
impl ShieldedLivenessBody {
    ...
    fn accepted_chunk(&self, outcome: &ShieldedAcceptedOutcome) -> Bytes {
        let body = match self.accepted_response_mode {
        ...
        let Some(client_model) = self.response_model_alias.as_deref() else {
            return body;
        };
    ...
    fn start_direct_relay(&mut self, outcome: ShieldedDirectRelayOutcome) -> Option<Bytes> {
        ...
        let stream = outcome.started.response.bytes_stream();
        self.direct_stream = Some(match self.response_model_alias.as_ref() {
            Some(client_model) => Box::pin(ResponseModelRewriteBody::new(
                stream,
                ResponseModelRewriteMode::OpenAiSse,
                client_model.clone(),
            )),
...
fn add_shielded_request_metadata(
    ...
    if shielded_chat {
        ...
        effective_liveness::add_request_metadata(metadata, liveness);
    }
